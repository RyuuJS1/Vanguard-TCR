...
async function toggleConexion() {
    ...
    if (!conectado) {
        ...
        try {
            let urlFinal = urlIngresada.includes(':') ? `http://${urlIngresada}/login` : `http://${urlIngresada}:5000/login`;
...
async function enviarAlServidor(accion, parametro) {
    ...
    try {
        let urlFinal = SERVER_IP.includes(':') ? `http://${SERVER_IP}/ejecutar` : `http://${SERVER_IP}:5000/ejecutar`;
...
async function getStats() {
    ...
    try {
        let urlFinal = SERVER_IP.includes(':') ? `http://${SERVER_IP}/status` : `http://${SERVER_IP}:5000/status`;
