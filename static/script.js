...
async function verificarPassword() {
    ...
    try {
        const response = await fetch(`https://${hostActual}:5000/login`, {
            method: 'POST',
        ...
        if (response.ok) {
            ...
            SERVER_IP = hostActual; // Guardamos la IP que funcionó

            // 1. Ocultamos el login
            document.getElementById('login-overlay').style.display = "none";
            
            // 2. Llenamos la IP pero NO llamamos a toggleConexion()
            document.getElementById('ipInput').value = hostActual; 
            
            // Mensaje opcional en la consola para guiar al usuario
            document.getElementById('console').innerHTML = "> Sesión iniciada. Configure la IP y presione Conectar.";
        ...
        alert("ERROR CRÍTICO: El navegador bloqueó la conexión HTTPS. Visita https://localhost:5000/status y acepta el certificado.");
    }
}
